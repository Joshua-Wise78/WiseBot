...
from server.paperless.paperless_utils import upload_document, retrieve_document
...
class Paperless(commands.Cog):
    ...
    async def cog_load(self):
        ...
        if self.client:
            ...
            print(
                "Paperless cog loaded, but Paperless client is currently not connected"
            )

    @app_commands.command(
        name="paperless-upload-document",
        description="Upload photo to paperless-ngx",
    )
    async def upload_document(
        self, interaction: discord.Interaction, document: discord.Attachment
    ):
        """
        ...
        try:
            ...
            await interaction.followup.send(
                f"Error uploaded document: {str(e)}"
            )
    ...
    async def retrieve_document(
        ...
        try:
            results = await retrieve_document(self, query=query, limit=limit)
            ...
            if not results:
                await interaction.followup.send(
                    f"ERROR 404: No documents found with {query}"
                )
                return
